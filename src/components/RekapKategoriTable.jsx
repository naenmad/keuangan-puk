...
export default function RekapKategoriTable({ monthlyData, availableYears }) {
  ...
  return (
    <div className="glass-panel rounded-3xl p-5 sm:p-7 mb-8 overflow-hidden">
      <div className="flex items-center justify-between pb-5 border-b border-slate-200 dark:border-borderCustom/40 mb-6">
        <div>
          <h2 className="text-lg sm:text-xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
            <Layers className="w-5 h-5 text-indigo-600 dark:text-accentPurple" />
        ...
        <table className="w-full text-left border-collapse text-xs sm:text-sm">
          <thead>
            <tr className="border-b-2 border-slate-300 dark:border-slate-800 bg-slate-100 dark:bg-surface/80 text-slate-700 dark:text-slate-300 font-bold uppercase text-[11px] tracking-wider">
              ...
              <th className="py-4 px-4 text-right font-bold text-rose-700 dark:text-rose-400 font-mono">
          ...
          <tbody className="divide-y divide-slate-200 dark:divide-slate-800/60 font-mono font-medium">
            {activeCategories.map((cat, idx) => {
              ...
              return (
                <tr key={cat} className="hover:bg-slate-50 dark:hover:bg-slate-800/40 transition-colors">
                  <td className="py-3.5 px-3 text-center text-slate-500 font-sans">{idx + 1}</td>
                  <td className="py-3.5 px-4 font-sans font-bold text-slate-900 dark:text-slate-200">{cat}</td>
                  {years.map(yr => {
                    const val = row.yearly[yr] || 0;
                    return (
                      <td key={yr} className="py-3.5 px-4 text-right text-slate-700 dark:text-slate-300">
                        {val > 0 ? formatRp(val) : <span className="text-slate-400">-</span>}
                      </td>
                    );
                  })}
                  <td className="py-3.5 px-4 text-right font-bold text-rose-700 dark:text-rose-400">
                    {row.total > 0 ? formatRp(row.total) : <span className="text-slate-400">-</span>}
                  </td>
                  <td className="py-3.5 px-4 text-center text-amber-700 dark:text-accentAmber font-bold font-mono">
                    {pct}%
                  </td>
                </tr>
              );
            ...
            {/* Total Row */}
            <tr className="bg-slate-100 dark:bg-slate-900 border-t-2 border-slate-400 dark:border-slate-700 font-bold text-sm">
              ...
              {years.map(yr => (
                <td key={yr} className="py-4 px-4 text-right text-rose-700 dark:text-rose-400 font-black font-mono">
                  {formatRp(yearTotals[yr] || 0)}
                </td>
              ))}
              <td className="py-4 px-4 text-right text-blue-700 dark:text-accentCyan text-base font-black font-mono">
                {formatRp(grandTotal)}
              </td>
